...
在这一部分，我们将创建真正的资源用户和帖子。我们将使用静态`ArrayList`来表示数据。
...
**第九步:**运行应用，在浏览器地址栏输入**本地主机:8080/用户**。它以 **JSON** 格式返回用户列表。
...
**第 11 步:**运行应用，在浏览器中键入**localhost:8080/users/{ id }**。它返回我们在 path 变量中传递的特定用户 id 的详细信息。
