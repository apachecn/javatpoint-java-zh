...
自 Java 8 以来，Java 引入了一个新的日期和时间API。java.time 包包含 Java 8 日期和时间类。
...
3.  **困难的时区处理:**使用经典的日期和日历类处理时区是困难的，因为开发人员应该为其编写逻辑。有了新的API，使用本地和区域数据/时间API可以轻松完成时区处理。

### Java 8 中新的日期时间API

新的日期API有助于克服上面提到的遗留类的缺点。它包括以下类别:
