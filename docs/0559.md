...
旧的 Doclet API被一个新的简化的API所取代，该接口利用了其他标准的现有API。在 Java 9 中，标准的 doclet 已经被重写，以使用新的 Doclet API。
...
过程API通过方法和接口得到了改进。这个API处理操作系统进程。
...
增加了标准的 XML 目录API，支持 OASIS(结构化信息标准)XML 目录 1.1 版的组织。该API由目录解析器组成，可用作 JAXP 处理器的内部组件。
...
@Deprecated 注释得到了改进，以提供关于API的状态和预期处置的更好信息。添加了以下新元素。
...
如果配置了堆栈审核器，这个堆栈审核API允许访问类对象。

### 纳斯霍恩解析器API

Java 增加了解析器API，允许在服务器端框架，特别是 IDEs 等中启用应用程序..
