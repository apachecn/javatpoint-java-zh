...
版本控制是API最重要和最困难的部分，因为它采用向后兼容的API。版本控制有助于我们在识别变更时更快地迭代。我们应该始终版本化我们的网络API。

考虑一个场景，其中我们有一个正在运行的网络API。用户正在使用该API。现在，我们希望在网络API中添加更多功能，但希望保持现有功能不变。可能很少有用户仍然想要使用旧的API，而其他用户想要具有新功能或扩展功能的新版本的API。这是网络API版本化产生的场景。

## 当我们需要版本控制时:

当我们在网络API中进行**突破性的更改**时，我们应该对API进行版本化。突破性变化包括:

*   一个或多个呼叫的响应数据格式的变化。
*   响应类型的变化。
*   删除API的任何部分。

中断更改应该总是导致API或内容响应类型的主要版本号发生更改。
...
接受头定义媒体类型和字符编码。我们还可以通过接受头传递网页API的版本信息，而无需更改网址。也称为媒体类型版本控制或内容协商或接受头。Github 使用接受头版本控制。用户无法在普通浏览器(chrome)中访问接受头版本。我们需要一个特殊的插件来访问他们的浏览器。
