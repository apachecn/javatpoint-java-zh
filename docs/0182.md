...
一些实现[类](object-class)是[缓存器](java-bufferedreader-class)、[缓存器](java-chararrayreader-class)、[过滤器读取器](java-filterreader-class)、[`InputStreamReader`](Input-from-keyboard-by-InputStreamReader)、管道缓存器、[字符串读取器](java-stringreader-class)
