...
> 原文:[https://www.javatpoint.com/java-switch](https://www.javatpoint.com/java-switch)

Java *开关语句*从多个条件中执行一条语句。这就像 [if-else-if](java-if-else) 阶梯语句。switch 语句适用于字节、短、整数、长、枚举类型、字符串和一些包装类型，如字节、短、整数和长。从 Java 7 开始，可以在 switch 语句中使用[字符串](java-string)。
...
*   每个 case 语句可以有一个 *break 语句*，这是可选的。当控制到达[中断语句](java-break)时，它在开关表达式后跳转控制。如果没有找到 break 语句，它将执行下一个案例。
*   案例值可以有一个*默认标签*，这是可选的。
...
![flow of switch statement in java](../Images/3f211b8600910f207dd9237dd0dbce4f.png)
...
我们可以在 Java 的其他 switch 语句中使用 switch 语句。它被称为嵌套开关语句。
