...
7)包含了一个新的方法 Java . lang . reflect . executable . getparameters，它允许您访问任何方法或构造函数的形式参数的名称。然而。默认情况下，类文件不存储形式参数名。在特定的。类文件，从而使反射 API 能够检索形式参数名，使用 javac 编译器的-parameters 选项编译源文件。
